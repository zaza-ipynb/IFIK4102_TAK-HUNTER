...
// if(!empty($_POST['nama']) && !empty($_POST['dates']) && !empty($_POST['jenis']) && !empty($_POST['harga']) && !empty($_POST['jpanitia']) && !empty($_POST['jpeserta']) && !empty($_POST['deskripsi'])):
//     $sql = "UPDATE events SET NamaEvent = :nama, JenisEvent = :jenis, JumlahPanitia = :jpanitia, JumlahPeserta = :jpeserta, Deskripsi = deskripsi, Dates = :dates, HargaTiket = :harga WHERE idEvent = :id";
//     $stmt = $conn->prepare($sql);
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, init ial-scale=1.0, shrink-to-fit=no">
        ...
        <div class="container">
            ...
            <nav id="sidebar">
                ...
                <div class="col-md-9" style="background-color: rgba(245,241,216,0.77);">
                    <form action ="UpdateEvent(EO).php" method="POST">
                        <h1 class="text-center">Update Event</h1>
                        <p class="text-left" style="color: #000000;">Nama Event<br>
                            <input class="form-control" type="text" name="nama" value="<?= $e['NamaEvent']; ?>" style="width: 820px;">
                        <br>Tanggal Event<br>
                            <input class="form-control" type="date" name="dates" value="<?= $e['Dates']; ?>" style="width: 410px;">
                        <br>Jenis Event<br>
                            <div class="radio">
                        ...
                        <br>Harga Tiket<br>
                            <input class="form-control" type="number" name="harga" value="<?= $e['HargaTiket']; ?>" style="width: 410px;">
                        <br>Jumlah Panitia<br>
                            <input class="form-control" type="number" name="jpanitia" value="<?= $e['JumlahPanitia']; ?>" style="width: 410px;">
                        <br>Jumlah Peserta<br>
                            <input class="form-control" type="number" name="jpeserta" value="<?= $e['JumlahPeserta']; ?>" style="width: 410px;">
                        <br>Deskripsi Event<br>
                            <textarea class="form-control" name="deskripsi" style="width: 820px;height: 180px;" ><?= $e['Deskripsi']; ?></textarea>
                        <!-- <br>Input Poster<br><input type="file"><br> --></p>
                        <p class="text-center">
                            <button class="btn btn-primary text-center" type="submit" style="background-color: #f4476b;">Update Event</button>
                        </p>
                    </form>
                </div>
            </div>
